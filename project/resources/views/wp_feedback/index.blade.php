...
<style>
    ...
    .user-link:hover .feedback-user-name {
        color: #4f46e5;
    }
    ...
    <div class="main-container">
        <div class="pd-ltr-20">
            <div class="feedback-stats">
                @foreach([
                    ['label' => 'Total Requests', 'value' => $stats['total'], 'color' => '#4f46e5'],
            ...
            <div class="feedback-panel">
                <div class="feedback-toolbar">
                    <form method="GET" action="{{ route('wp_feedback.index') }}">
                        ...
                        <div class="feedback-field" style="flex: 0 0 auto; min-width: 0;">
                            ...
                            <a href="{{ route('wp_feedback.index') }}" class="btn btn-light border">Reset</a>
                        </div>
                    </form>
                </div>
                        ...
                        <tbody>
                            @forelse ($feedbackRequests as $i => $req)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($user && $user->email)
                                            <a href="{{ url('user_detail/' . $user->email) }}" class="user-link">
                                                <span class="feedback-user-name">{{ $user->name ?? 'Unknown User' }}</span>
                                                <span class="feedback-user-meta">{{ $user->email }}</span>
                                            </a>
                                        @else
                                            <span class="feedback-user-name">{{ $user?->name ?? 'Unknown User' }}</span>
                                    ...
                                    <td>{{ $req->contact_no ?: ($user?->number ?? '—') }}</td>
                                    <td>{{ $req->purchase_id }}</td>
                                    <td>
                                        <span class="status-pill {{ $req->display_status }}">
                                            {{ ucfirst($req->display_status) }}
                                        </span>
                                    </td>
                                    <td>
                                        @if ($response)
                                            @for ($s = 1; $s <= 5; $s++)
                                                <span style="color: {{ $s <= $response->rating ? '#f59e0b' : '#cbd5e1' }}; font-size: 15px;">★</span>
                                            @endfor
                                        @else
                                    ...
                                    <td style="white-space: nowrap;">{{ $response?->submitted_at?->format('d/m/Y H:i') ?? '—' }}</td>
                                    <td style="position: relative;white-space: nowrap;">
                                        <input type="checkbox" class="followup-switch switch-btn me-3"
                                               ...
                                               @if ($req->followup_call == 1) checked @endif disabled style="pointer-events: none; opacity: 0.5;" />
                                        @if (!empty($req->followup_note) || !empty($req->followup_label))
                                            <i class="fa-solid fa-circle-info info-icon"
                                               data-id="{{ $req->id }}"
                                               data-note="{{ $req->followup_note }}"
                                               data-label="{{ $req->followup_label }}"
                                               data-label-display="{{ $followupLabels[$req->followup_label] ?? $req->followup_label }}"></i>
                                        @endif
                                    </td>
                                    <td style="word-break: keep-all">{{ \App\Http\Controllers\Utils\RoleManager::getUploaderName($req->emp_id) }}</td>
                                    <td>
                                        @if ($req->has_feedback_response)
                                            <button class="btn btn-outline-primary view-btn view-feedback-btn"
                                                data-rating="{{ $response->rating }}"
                                                data-feedback="{{ e($req->display_feedback ?? '—') }}"
                                                data-suggestions="{{ e($req->display_suggestions ?? '—') }}"
                                                data-user="{{ e($user?->name ?? 'Unknown User') }}"
                                                data-email="{{ $user?->email ?? '' }}"
                                                data-submitted="{{ $response?->submitted_at?->format('d/m/Y H:i') ?? '—' }}">
...
<div class="modal fade" id="feedbackDetailModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p><strong>User:</strong> <a href="#" id="modal-user-link" style="text-decoration: none; font-weight: 600; color: #4f46e5;"><span id="modal-user"></span></a></p>
    ...
    $(document).off("change", ".followup-switch").on("change", ".followup-switch", function() {
        ...
        if (isChecked) {
            ...
            if (confirm("Are you sure you want to uncheck this follow-up?")) {
                $.ajax({
                    url: "{{ route('wp_feedback.followupUpdate') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: id,
...
$(document).on('click', '.view-feedback-btn', function () {
    ...
    if (email) {
        $('#modal-user-link').attr('href', '{{ url("user_detail") }}/' + email).show();
    } else {
        $('#modal-user-link').removeAttr('href').css('cursor', 'default');
    }
