                        ...
                        <tbody>
                            @foreach ($offers as $offer)
                                <tr id="row_{{ $offer->id }}">
                                    <td>{{ $offer->id }}</td>
                                    <td>
                                        <button style="border:none" onclick="pinned_click('{{ $offer->id }}')">
        ...
        <div class="modal-content">
            <form id="offerForm">@csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="d-flex">
                        <input type="number" class="form-control m-2 w-50" name="duration_time_value"
                    ...
                    <div class="d-flex">
                        <input type="number" class="form-control m-2 w-100" name="frequency_duration_value"
                    ...
                    <div class="d-flex">
                        <input type="number" class="form-control m-2 w-50" name="force_show_duration_value"
...
<script>
    ...
    $(document).on("click", ".edit-offer", function() {
        let id = $(this).data("id");
        $.get("{{ url('offer-pop-up') }}/" + id + "/edit", function(res) {
            $("#id").val(res.id);
            $("#enable_offer").prop("checked", res.enable_offer == 1);
            $("#enable_force").prop("checked", res.enable_force == 1);

            $("#duration_time_value").val(res.duration_time.value);
            $("#duration_time_unit").val(res.duration_time.unit);
    ...
    $("#offerForm").on("submit", function(e) {
        ...
        $.post("{{ route('offer-pop-up.store') }}", formData, function(res) {
            if (res.status) {
    ...
    const setEnableUrl = "{{ route('offer-pop-up.set-enable', ['id' => ':id']) }}";
