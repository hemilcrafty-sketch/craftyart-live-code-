        ...
        <div class="system-card">
            <div class="system-header">
                <div>
                    ...
                    <p class="system-subtitle">Manage what designers create (Freelancer / crafty_creator)</p>
                </div>
                ...
                <table class="system-table table">
                    ...
                    <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn-action btn-edit"
                                        onclick="openEditModal({{ $type->id }}, '{{ addslashes($type->name) }}', '{{ addslashes($type->description ?? '') }}', {{ $type->sort_order }})"><i
                                            class="fa fa-edit"></i></button>
                                    <form action="{{ route('designer_system.types.toggle', $type->id) }}" method="POST"
                                        style="display:inline;">@csrf
                                        <button type="submit" class="btn-action btn-toggle"><i
                                                class="fa fa-toggle-{{ $type->is_active ? 'on' : 'off' }}"></i></button>
                                    </form>
                                    <form action="{{ route('designer_system.types.delete', $type->id) }}" method="POST"
                                        style="display:inline;" onsubmit="return confirm('Delete this type?');">@csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7">
                                    <div class="empty-state">No designer types. Add one above.</div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
