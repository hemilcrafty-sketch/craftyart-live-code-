...
class SearchApiController extends ApiController
{
    ...
    public function exactKeywordTemplates($rates, $keywords, $limit, $excludeTemplate = null): array
    {
        ...
        $categoryIds = $itemData->pluck('new_category_id')->unique();
...
//            'isLastPage' => count($item_rows) < $limit,
    ...
    public function searchTemplates1($rates, $keywords, $page, $cat_id, $limit, $excludeIdName = "love", $ratio = null): array
    {
        ...
        $sql .= "WHEN SOUNDEX(related_tags) = SOUNDEX('" . addslashes($keywords) . "') THEN " . (count($desc_array) + 2) . " ";
        $sql .= "WHEN SOUNDEX(related_tags) = SOUNDEX('" . implode(' ', array_reverse($desc_array)) . "') THEN " . (count($desc_array) + 1) . " ";


        foreach ($desc_array as $index => $keyword) {
            $sql .= "WHEN SOUNDEX(related_tags) = SOUNDEX('" . addslashes($keyword) . "') THEN " . (count($desc_array) - $index) . " ";
        }

        $sql .= "ELSE 0 END AS keyword_match_priority
                        FROM designs
                        WHERE id_name != '" . $excludeIdName . "'
                        AND new_category_id " . $CatCondition . " " . $catId . "
                        AND ratio " . $ratioCondition . " " . $ratio . "
                        AND status " . $status_condition . " " . $status . "
                        AND (
                            SOUNDEX(related_tags) = SOUNDEX('" . addslashes($keywords) . "')
                            OR SOUNDEX(related_tags) = SOUNDEX('" . implode(' ', array_reverse($desc_array)) . "') ";


        foreach ($desc_array as $keyword) {
            $sql .= "OR SOUNDEX(related_tags) = SOUNDEX('" . addslashes($keyword) . "') ";
        }
