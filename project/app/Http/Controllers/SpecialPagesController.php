...
class SpecialPagesController extends AppBaseController
{
    ...
    public function index(Request $request): Factory|View|Application
    {
        ...
        $noIndexStats = $this->noIndexListingStats(SpecialPage::class);

        $query = SpecialPage::query();
        $this->applyNoIndexFilter($request, $query);

        $specialPages = $this->applyFiltersAndPagination($request, $query, $searchableFields);

        return view("special_pages.index", compact('specialPages', 'searchableFields', 'noIndexStats'));
    }
    ...
    {
        Log::info("SpecialPage Update Started", [
            'id' => $request->id,
            'slug' => $request->page_slug,
            'user_id' => Auth::user()->id,
            'contents_size' => $request->has('contents') ? strlen($request->contents) : 'MISSING'
        ]);
        // dd($request->cat_id);
        ...
        if ($request->id) {
            ...
            if (!roleManager::isAdmin(Auth::user()->user_type) && roleManager::isAdminOrSeoManager(Auth::user()->user_type)) {
                ...
                if ($canonicalError) {
                    ...
                    $res->save();
                    // Fall through to allow full page update
                }
            }
        }

        $seoAssignerID = (isset($res) && $res->newCategory) ? $res->newCategory->seo_emp_id : 0;
        ...
        if ($request->id) {
            $pathOrContent = SpecialPage::where('id', $request->id)->value('contents');
            $contentsArray = self::getJsonContent($pathOrContent);
        ...
        try {
            ...
            if (isset($request->contents)) {
                Log::info("Processing Contents", ['path' => $contentPath, 'raw_size' => strlen($request->contents)]);
                $contents = ContentManager::getContents($request->contents, $fldrStr, $availableImage, $availableVideo);
                
                // If content processing returned nothing/false, but request had content, use raw request to avoid loss
                if (($contents === "" || $contents === "[]" || $contents === false) && !empty($request->contents) && $request->contents !== "[]") {
                    Log::warning("Content processing returned empty/false. Falling back to raw contents.", ['id' => $request->id]);
                    $contents = $request->contents;
                }

                $contentPath = $contentPath ?? 'sp/' . $fldrStr . '/jn/' . StorageUtils::getNewName() . ".json";
                Log::info("Saving Content to Storage", ['path' => $contentPath, 'final_size' => is_string($contents) ? strlen($contents) : 0]);
                StorageUtils::put($contentPath, $contents);
                $data['contents'] = $contentPath;
            }

            if (isset($request->faqs)) {
                $decodedFaqs = json_decode($request->faqs);
                if (json_last_error() === JSON_ERROR_NONE && $decodedFaqs !== null) {
                    $faqsPath = $faqsPath ?? 'sp/' . $fldrStr . '/fq/' . StorageUtils::getNewName() . ".json";
                    $faqsData['title'] = $data['faqs_title'] ?? '';
                    $faqsData['faqs'] = $decodedFaqs;
                    StorageUtils::put($faqsPath, json_encode($faqsData));
                    $data['faqs'] = $faqsPath;
                } elseif (!empty($request->faqs) && $request->faqs !== "[]") {
                    Log::error("FAQ JSON Decode Failed", ['raw_faqs' => $request->faqs]);
                    // If decode failed but it's not empty, it might be legacy or slightly malformed JSON, try to save raw or keep old
                    // For now, if it's there but invalid, we avoid overwriting with a 'null' object
                }
            }
