...
//        if ($this->isTester()) $this->uid = 'kk4Qk6xR8whuK4xNP5rYM4cy8ad2';
        ...
        if ($user->hoc) {
            ...
            if ($page == 1) {
                ...
                $pending = (clone $query)->where('user_id', $this->uid)->where('is_live', 0)->paginate($limit, ['*'], 'page', $page);
                ...
                $datas[] = [
                    "title" => "Pending",
                    "datas" => $this->processThumbs($pending->items()),
                    "length" => $pending->total(),
                    "type" => 0,
                    "pageNo" => $page,
                    "isLastPage" => $pending->currentPage() >= $pending->lastPage()
                ];
