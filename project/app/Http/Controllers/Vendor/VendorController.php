...
class VendorController extends ApiController
{
        ...
        $data = RevenueHistory::query()
            ...
            ->selectRaw("
                SUM(CASE 
                    ...
                    AND type NOT IN ('withdraw', 'bank_validation') 
                    THEN vendor_amount 
                    ELSE 0 
                END) as earnings,
                ...
                SUM(CASE 
                    WHEN status = 'completed'
                    AND type IN ('withdraw', 'bank_validation') 
                    THEN vendor_amount 
                    ELSE 0 
                END) as withdraw
            ")
            ->first();
        $data2 = RevenueHistory::query()
            ->where('user_id', (string) $userId)
            ->where('vendor_type', (string) $type)->get();
        if (!$data) {
            return [
                'earnings' => 0,
                'withdraw' => 0,
    ...
    function getReferralDashboard(Request $request): array|string
    {
        ...
        $referralStats = $user ? self::affiliateSummary($this->uid, 'affiliate') : [];
        ...
        $peopleReferred = $user ? UserData::where('referral_user_id', $user->id)->count() : 0;

        // Calculate commission paid
        $commissionPaid = $totalWithdrawn;

        // Calculate average per year
        $userCreatedAt = \Carbon\Carbon::parse($user->created_at ?? now());
        $now = \Carbon\Carbon::now();
        $yearsSinceCreation = max(1, $userCreatedAt->diffInYears($now) ?: 1);
        $averagePerYear = $yearsSinceCreation > 0 ? round($totalEarned / $yearsSinceCreation, 2) : 0;

        // Generate referral link
        $baseUrl = "https://www.craftyartapp.com";
        $referralLink = $user ? $baseUrl . '?REFERRALCODE=' . strtoupper($user->refer_id) : null;
        ...
        return $this->successed(
            ...
            datas: [
                ...
                'refer_id' => $user->refer_id ?? null,
                'referral_code' => $user->refer_id ?? null,
    ...
    private function getHistoryByType(Request $request, string $affiliateType): array|string
    {
        try {
            ...
            // 1. Fetch Paid/Withdrawal History
            $referralHistory = RevenueHistory::whereUserId($this->uid)
                ->where('vendor_type', $affiliateType)
                ->with(['purchaseHistory.userData', 'purchaseUser'])
                ->orderBy('created_at', 'desc')
                ->get();

            $referralData = [];

            // 2. Identify Pending Referrals (only for affiliate type)
            if ($affiliateType === 'affiliate') {
                $user = UserData::whereUid($this->uid)->first();
                if ($user) {
                    // Collect UIDs that already have paid entries to filter them out from pending
                    $paidUserUids = $referralHistory->pluck('purchase_user_id')->filter()->unique()->toArray();

                    // Find users who have this user as referral_user_id but no paid entry yet
                    $referredUsers = UserData::where('referral_user_id', $user->id)
                        ->whereNotIn('uid', $paidUserUids)
                        ->orderBy('created_at', 'desc')
                        ->get();

                    foreach ($referredUsers as $refUser) {
                        $referralData[] = [
                            'string_id' => 'pending_' . $refUser->uid,
                            'type' => 'signup',
                            'is_withdraw' => false,
                            'vendor_amount' => 0,
                            'vendor_percentage' => 0,
                            'purchase_amount' => 0,
                            'purchase_user_id' => $refUser->uid,
            ...
            // 3. Add Paid Entries
            foreach ($referralHistory as $referralItem) {
                $isWithdraw = $referralItem->type === 'withdraw';
                ...
                if (!$isWithdraw) {
                    if ($affiliateType === 'affiliate') {
                        // Buyer who triggered the commission
                        $buyer = $referralItem->purchaseUser;
                        if ($buyer) {
                            $data['purchase_user'] = [
                                'uid' => $buyer->uid,
                                'name' => $buyer->name,
                        ...
                        // Type freelancer: show purchaseBy
                        $purchaseUser = $referralItem->purchaseHistory?->userData;
                        if ($purchaseUser) {
                            $data['purchaseBy'] = [
                                'name' => $purchaseUser->name,
            ...
            // 4. Sort combined data by created_at desc
            usort($referralData, function ($a, $b) {
            ...
            // 6. Paginate result
            $total = count($referralData);
            if ($request->get('dummy_data') == 1) {
                $lastPage = 1;
                $perPage = 20;
                $paginatedData = $referralData;
            } else {
                $lastPage = (int)ceil($total / $perPage);
                $offset = ($page - 1) * $perPage;
                $paginatedData = array_slice($referralData, $offset, $perPage);
            }
