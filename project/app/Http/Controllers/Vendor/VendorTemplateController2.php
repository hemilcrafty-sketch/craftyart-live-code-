...
class VendorTemplateController extends ApiController
{
    public function addOrUpdateVendorDetails(Request $request): array|string
    {
        ...
        if ($error) return $this->failed(msg: $error);
    ...
    public function getVendorHome(Request $request): array|string
    {
        ...
        if ($page === 1) {
            ...
            $filters = collect($data)->only([
                'colors', 'interests', 'sizes', 'styles', 'languages', 'religions', 'themes'
            ]);
    ...
    public function getVendorTemplate(Request $request): array|string
    {
        ...
        if (is_string($resolved)) return $this->failed(msg: $resolved);
        ['uid' => $targetUid, 'user' => $targetUser] = $resolved;
        $query = VendorTemplate::where('user_id', $targetUid);

        if ($request->filled('search')) {
            $searchTerm = $request->input('search');
            $matchingDesignIds = Design::where(function ($q) use ($searchTerm) {
        ...
        $designQuery = Design::whereIn('string_id', $templateIds);

        // Apply all filters (including category) via getFilterQuery
        self::getFilterQuery($designQuery, $filter);

        $designs = $designQuery->get()->keyBy('string_id');
        ...
        $datas = $vendorTemplates->getCollection()->map(function ($vt) use ($designs, $categoriesMap, $rates, $targetUid) {
            ...
            if ($catRow) $catLink = $catRow->cat_link;

            $itemData = HelperController::getItemData(
                uid: $targetUid, catRow: $catRow, item: $item,
                thumbArray: json_decode($item->thumb_array, true) ?? [],
                catLink: $catLink, rates: $rates
            );
        ...
        if ($page == 1) {
            ...
            $allVendorTemplateIds = VendorTemplate::where('user_id', $targetUid)
                ->pluck('template_id');

            $designs = Design::whereIn('string_id', $allVendorTemplateIds)
                ->select('new_category_id', 'is_premium', 'is_freemium')
                ->get();

            $categoryIds = $designs->pluck('new_category_id')->unique()->filter()->values();
            ...
            $templateCounts = [
                'total' => $designs->count(),
                'free' => $designs->where('is_premium', 0)->count(),
                'freemium' => $designs->where('is_premium', 1)->where('is_freemium', 1)->count(),
                'premium' => $designs->where('is_premium', 1)->where('is_freemium', 0)->count(),
            ];
        }
...
//        ];
        ...
        return $this->successed(msg: 'Loading Success!', datas: [
            ...
            "template_count" => $templateCounts ? [
                ...
                "freemium" => (int)$templateCounts['freemium'],
                "premium" => (int)$templateCounts['premium'],
            ] : [],
    ...
    //         $templateCounts = [
    //             'total' => $designs->count(),
    //             'free' => $designs->where('is_premium', 0)->where('is_freemium', '!=', 1)->count(),
    //             'freemium' => $designs->where('is_freemium', 1)->count(),
    //             'premium' => $designs->where('is_premium', 1)->where('is_freemium', '!=', 1)->count(),
    //         ];
    //     }
    ...
    // Toggle: if template exists in vendor account → remove it, else → add it
    public function toggleVendorTemplate(Request $request): array|string
    {
        ...
        if (!$template) return $this->failed(msg: "Template Not Found");
...
//                $religions = $this->getCounts(Religion::class, ['id', 'religion_name as name', 'id_name'], 'religion_id');
    ...
    private function resolveTenantUid(Request $request): array|string
    {
        ...
        if (empty($requestDomain) || in_array($requestDomain, $craftyDomains)) {
            if ($this->isFakeRequestAndUser($request)) return "UnAuthorize";
            $error = self::isValidVendor();
        ...
        if (!$tenant) return $requestDomain;
    ...
    {
        if ($userName) $userData = UserData::with('latestTransactionLog')->whereUserName($userName)->first();
        else $userData = UserData::with('latestTransactionLog')->whereUid($uid ?? $this->uid)->first();

        if (!$userData) return ($uid || $userName) ? "User Not Found" : "UnAuthorize";

        if (!$userData->latestTransactionLog) return "Subscription Not Found";

        if ($userData->latestTransactionLog->expired_at < now()) return "Subscription is Expired";
