...
class VendorTemplateController extends ApiController
{
    public function addOrUpdateVendorDetails(Request $request): array|string
    {
        ...
        $vendorDetails->meta_title = $request->input('meta_title');
        $vendorDetails->meta_description = $request->input('meta_description');
    ...
    public function getVendorHome(Request $request): array|string
    {
        ...
        // ⚡ FAST LOOKUP (Fetch full objects to check status and preserve data)
        $vendorTemplates = VendorTemplate::where('user_id', $this->uid)
            ->get()
            ->keyBy('template_id');

        $rates = RateController::getRates();
        // 🎯 DATA TRANSFORM
        $item_rows = collect($templates->items())->map(function ($item) use ($categories, $rates, $vendorTemplates) {
            ...
            // ⚡ O(1)
            $vt = $vendorTemplates->get($itemData['string_id']);
            $itemData['is_vendor_added'] = (bool) ($vt->status ?? false);
            $itemData['vendor_status'] = (bool) ($vt->status ?? false);
            $itemData['is_Vendor_template_data_added'] = (bool) $vt;

            if ($vt) {
                $itemData['description'] = $vt->description;
                $itemData['long_description'] = $vt->long_description;
                $itemData['vendor_template_data'] = [
                    'id' => $vt->id,
                    'user_id' => $vt->user_id,
                    ...
                    'long_description' => $vt->long_description,
                    'is_premium' => (bool) $vt->is_premium,
                    'no_index' => (bool) $vt->no_index,
                    'status' => (bool) $vt->status,
                ];
            }
    ...
    public function getVendorTemplate(Request $request): array|string
    {
        ...
        $query = VendorTemplate::where('user_id', $targetUid)->where('status', 1);
        ...
        if ($searchTerm || !empty($filter)) {
            ...
            $allVendorTemplateIds = VendorTemplate::where('user_id', $targetUid)->where('status', 1)->pluck('template_id');
        ...
        $datas = $vendorTemplates->getCollection()->map(function ($vt) use ($designs, $categoriesMap, $rates, $targetUid) {
            ...
            if ($itemData) {
                ...
                $itemData['is_vendor_added'] = (bool)$vt->status;
                $itemData['vendor_status'] = (bool)$vt->status;
                $itemData['is_Vendor_template_data_added'] = true;
            } else {
                $itemData['is_Vendor_template_data_added'] = false;
            }
        ...
        if ($page == 1) {
            ...
            $allVendorIds = VendorTemplate::where('user_id', $targetUid)->where('status', 1)->pluck('template_id');
            ...
            $filteredQuery = VendorTemplate::where('user_id', $targetUid)->where('status', 1);
            if ($searchTerm || !empty($filter)) {
    ...
    /**
     * Get a single vendor template by its slug (public-facing)
     */
    public function getVendorTemplateBySlug(Request $request): array|string
    {
        $slug = $request->input('slug');
        if (!$slug) {
            return $this->failed(msg: "slug is required");
        }

        $resolved = $this->resolveTenantUid($request);
        if (is_string($resolved))
            return $this->failed(msg: $resolved);
        ['uid' => $targetUid] = $resolved;

        $vt = VendorTemplate::where('slug', $slug)
        ...
        $item = Design::where('string_id', $vt->template_id)->first();
        if (!$item) {
            return $this->failed(msg: "Design not found");
        }

        $catRow = NewCategory::find($item->new_category_id);
        $catLink = $catRow->cat_link ?? (HelperController::$webPageUrl . "templates/p/" . $item->id_name);
        $rates = RateController::getRates();
        ...
        return $this->successed(msg: "Template found", datas: [
            "is_vendor_details_added" => (bool) $vendorDetails,
            "vendor_details" => $vendorDetails,
            "template" => $itemData,
        ]);
    ...
    public function toggleVendorTemplate(Request $request): array|string
    {
        ...
        $longDescription = $request->input('long_description') ?? $request->input('long_desc');
        $isPremium = $request->input('is_premium', 0);
        $noIndex = $request->input('no_index', 0);
        $slug = $request->input('slug');
        ...
        if ($existing) {
            ...
            // Fix: Only update fields if they are present in the request to avoid overwriting with defaults
            if ($request->has('price')) $existing->price = $request->input('price');
            if ($request->has('currency')) $existing->currency = $request->input('currency');
            if ($request->has('title')) $existing->title = $request->input('title');
            if ($request->has('description') || $request->has('desc')) $existing->description = $description;
            if ($request->has('long_description') || $request->has('long_desc')) $existing->long_description = $longDescription;
            if ($request->has('is_premium')) $existing->is_premium = $isPremium;
            if ($request->has('no_index')) $existing->no_index = $noIndex;

            if ($slug && $slug !== $existing->slug) {
                if (VendorTemplate::where('slug', $slug)->where('id', '!=', $existing->id)->exists()) {
                    return $this->failed(msg: "Slug " . $slug . " is already in use");
                }
                $existing->slug = $slug;
            }
            ...
            $msg = $existing->status == 1 ? 'Template added to your account' : 'Template removed from your account';
            return $this->successed(msg: $msg);
        }
        ...
        $vendorTemplate->title = $title;
        $vendorTemplate->description = $description;
        $vendorTemplate->long_description = $longDescription;
        $vendorTemplate->is_premium = $isPremium;
        $vendorTemplate->no_index = $noIndex;
        $vendorTemplate->status = 1;

        if ($slug) {
            if (VendorTemplate::where('slug', $slug)->exists()) {
                return $this->failed(msg: "Slug " . $slug . " is already in use");
            }
            $vendorTemplate->slug = $slug;
        }
    ...
    public function bulkToggleVendorTemplate(Request $request): array|string
    {
        if ($this->isFakeRequestAndUser($request)) {
            return $this->failed(msg: "Unauthorized");
        }
        ...
        $templateIds = $request->input('ids') ?? $request->input('id');
        $status = $request->input('status');

        if (!$templateIds || !is_array($templateIds)) {
            return $this->failed(msg: "ids array is required");
        }
        ...
        foreach ($templateIds as $templateId) {
            ...
            if ($existing) {
                // If status is provided, use it. If not, toggle (like toggleVendorTemplate)
                if (isset($status)) {
                ...
                if ($template) {
                    ...
                    $vendorTemplate->user_id = $this->uid;
                    $vendorTemplate->template_id = $templateId;
                    $vendorTemplate->status = isset($status) ? $status : 1;
                    $vendorTemplate->price = 0;
                    $vendorTemplate->currency = "INR";
                    $vendorTemplate->save();
                    $count++;
                }
            }
        ...
        return $this->successed(msg: $count . ' templates updated successfully');
    }
