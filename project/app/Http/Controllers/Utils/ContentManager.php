...
class ContentManager extends Controller
{
    ...
    public static function getContents($contents, $fldrStr, $availableImage = [], $availableVideo = []): bool|string
    {
        ...
        if (json_last_error() !== JSON_ERROR_NONE || $decodedContents === null) {
            Log::error("Content JSON Decode Failed", ['error' => json_last_error_msg(), 'input_start' => substr($contents, 0, 100)]);
            if (!empty($contents) && $contents !== "[]" && $contents !== "null") {
                // If decode failed but input wasn't empty, return original to avoid data loss
                return $contents;
            }
            return "[]";
        }
        $contentsArray = $decodedContents;
        foreach ($contentsArray as $contentKey => $items) {
            if ($items !== null && (isset($items->type) || (is_array($items) && isset($items['type'])))) {
        ...
        $result = json_encode($contentsArray);
        if ($result === false) {
             Log::error("Content JSON Encode Failed", ['error' => json_last_error_msg()]);
             // If encode fails, return original or log
             return is_string($contents) ? $contents : "[]";
        }
        Log::info("Content Processed Successfully", ['items_count' => count($contentsArray)]);
        return $result;
    }
