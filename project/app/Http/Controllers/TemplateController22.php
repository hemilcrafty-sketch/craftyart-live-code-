...
class TemplateController extends AppBaseController
{
    ...
    public function update_seo(Request $request, Design $design)
    {
        ...
        if ($post_thumb) {
            if ($res->post_thumb) StorageUtils::delete($res->post_thumb);
        ...
        if ($category) {
            ...
            if (!empty($category->parent_category_id) && $category->parent_category_id != 0) {
                ...
                if ($parent) {
                    ...
                    $childCategoryIds = NewCategory::where('parent_category_id', $category->parent_category_id)->pluck('id');
                    if ($childCategoryIds->isNotEmpty()) {
                        $designCount = Design::whereIn('new_category_id', $childCategoryIds)
                            ->whereStatus(1)
                            ->count();
