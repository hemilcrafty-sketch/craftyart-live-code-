...
class UnifiedLeadsController extends AppBaseController
{
    ...
    protected function buildPurchaseHistoryQuery($request, bool $isSalesEmployee, int $userId): Builder|MasterPurchaseHistory
    {
        ...
        $query = MasterPurchaseHistory::with(['userData.personalDetails', 'deletedUser', 'employee', 'unifiedSupport', 'wpFeedbackRequest', 'wpFeedbackResponse', 'transactionLog'])
            ->select([
                'id',
                'user_id',
                'product_id',
                ...
                'created_at',
                'updated_at'
            ])->whereIn('product_type', $subscriptionTypes);
        ...
        $automationDb = config('database.connections.crafty_automation_mysql.database');
        $query->orderBy('status', 'desc')
            ->orderByRaw("COALESCE((SELECT event_date FROM {$automationDb}.unified_support WHERE purchase_history_id = purchase_history.id LIMIT 1), created_at) DESC")
            ->orderBy('id', 'desc');
...
//        }
        ...
        return [
            ...
            'amount' => $purchase->amount_with_symbol,
            'amount_numeric' => ($purchase->transactionLog->paid_amount ?? $purchase->paid_amount ?? $purchase->amount ?? 0),
            ...
            'event_date' => $support && $support->event_date ? $support->event_date->format('Y-m-d H:i:s') : ($purchase->created_at ? (is_string($purchase->created_at) ? $purchase->created_at : $purchase->created_at->format('Y-m-d H:i:s')) : null),
            'source_type' => $support->source_type ?? 'purchase_history',
        ];
    }
