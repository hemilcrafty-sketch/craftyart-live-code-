...
class NewRecentExpireController extends Controller
{
    ...
    public function index(Request $request): Factory|View|Application
    {
        ...
        $query = \App\Models\Revenue\MasterPurchaseHistory::query()
        ...
        if ($isSalesEmployee) {
            ...
            $query->where(function ($q) use ($userId) {
                $q->where('purchase_history.emp_id', $userId)
                    ->orWhere(function ($sub) {
                        $sub->where('purchase_history.emp_id', 0)
                            ->whereRaw('(purchase_history.id % 4) = 0');
                    });
            });
        }
        ...
        $datas['packageArray'] = SubPlan::all();
