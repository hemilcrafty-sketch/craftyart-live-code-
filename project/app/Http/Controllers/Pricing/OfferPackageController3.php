...
class OfferPackageController extends AppBaseController
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $isCustom = $request->sub_plan_id === self::SUB_PLAN_CUSTOM;
        if ($isCustom) {
            $request->validate(['custom_days' => 'required|integer|min:1']);
        } else {
            $request->validate(['duration_id' => 'required|string']);
        }

        $slugPattern = '/^[a-zA-Z0-9]+(?:[_-][a-zA-Z0-9]+)*$/';
        $slugs = [];
        foreach ((array) $request->input('offer_slugs', []) as $i => $s) {
            if (! is_string($s)) {
                continue;
            }
            $t = trim($s);
            ...
            if (preg_match($slugPattern, $t) !== 1) {
                throw ValidationException::withMessages([
                    'offer_slugs.'.$i => ['Use only letters, numbers, hyphens, and underscores — no spaces, ?, #, /, or other special characters.'],
                ]);
            }
            $slugs[] = $t;
        }
        if ($slugs === []) {
            throw ValidationException::withMessages(['offer_slugs' => ['Add at least one slug.']]);
        }

        $urls = [];
        foreach ((array) $request->input('offer_urls', []) as $i => $u) {
            if (! is_string($u)) {
                $urls[] = '';
            ...
            if ($trimmed !== '' && ! filter_var($trimmed, FILTER_VALIDATE_URL)) {
                throw ValidationException::withMessages([
                    'offer_urls.'.$i => ['Enter a valid URL (include https://).'],
                ]);
            }
            $urls[] = $u;
        }
        ...
        $instructions = null;
        if ($request->boolean('enable_instructions')) {
            $t = $request->input('instruction_text');
            $instructions = is_string($t) ? $t : '';
        }

        $data = [
            'plan_id'          => $request->plan_id,
            // 'sub_plan_id'      => $isCustom ? null : $request->sub_plan_id,
