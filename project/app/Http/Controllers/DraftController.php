        ...
        try {
            ...
            if ($w && $h) {
                if ($user_data) {
                    ...
                    $templateLimit = PlanLimitHelper::getPlanLimit($request, 'access_template', $this->uid, false, false);
                    if ($templateLimit['excessed']) {
                        return $this->failed(msg: $templateLimit['msg'], datas: ['limit_exceeded' => $templateLimit]);
                    }
...
//        $isBetaEditor = DomainChecker::isBetaEditor($request);
        ...
        if (!$draft_data) {
            ...
            if ($user_data) {
                ...
                $isExists = PurchaseHistory::where('user_id', $user_data->uid)->where('product_id', $itemData->string_id)->where('product_type', 0)->wherePaymentStatus(1)->exists();
    ...
    function saveData(Request $request, bool $showDecoded = false): array|string
    {
        ...
        if (empty($draft_save_data)) {
            Draft::where("string_id", $draft_id)->where('user_id', $uid)->update(array('name' => $post_name));
            return $this->successed(msg: "Draft Updated Successfully", datas: ['type' => 0], showDecoded: $showDecoded);
        }

        if ($draft_id == null || $draft_thumbs == null) {
            return $this->failed(statusCode: 502, msg: "Parameters Missing!", datas: ['type' => 0], showDecoded: $showDecoded);
        }
