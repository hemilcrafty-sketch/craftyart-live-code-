...
//        $user['cancel_sub'] = false;
        ...
        $subData = $this->getUserSubHistory($request, $userData);
        $currenSub = $subData['current'];

        // $subExists = false;
        // if ($currenSub) {
        //     $userSub = UserSubscriptions::whereUserId($userData->uid)->whereGatewaySubscriptionId($currenSub['subId'])->latest()->first();
        //     if ($userSub) $subExists = !($userSub->status == "cancelled");
        // }
        $subExists = false;
        $userSub = UserSubscriptions::whereUserId($userData->uid)->latest()->first();
        if ($userSub) $subExists = !($userSub->status == "cancelled");

        $user['cancel_sub'] = $subExists;
        // // force override for specific UID
        // if ($userData->uid === 'MPCoXRtfnnS174tl67DTsEjfz6P') {
        //     $user['cancel_sub'] = true;
        // } else {
        //     $user['cancel_sub'] = $subExists;
        // }

        if ($currenSub) {
            $amount = $currenSub["amount"];
            $billing_date = $currenSub["billing_date"];
            if ($subExists) $user['expiry_msg'] = "Your next billing date is $billing_date";
            else $user['expiry_msg'] = "Your current plan $amount is end on $billing_date";
        }
...
//        }
        ...
        $response['isNewUser'] = $isNewUser;
        if (!$minimalResponse) {
            $response['currentPlan'] = $currenSub;
            $response['subsHistory'] = $subData["history"];
            $response['purHistory'] = $this->getUserPurchaseHistory($userData);
        }
    ...
    private function getUserSubHistory(Request $request, UserData $user_data): array|null
    {
        ...
        if ($transLog != null) {
            ...
            if ($subRow) {
                ...
                $singleData['subId'] = $transLog->subscription_id;
