 ...
 * @property NewCategory[] $subcategories
 * @property array $parent
 * @property string $cat_link
 * @property-read Collection<int, NewCategory> $children
 * @property-read int|null $children_count
 * @property-read Collection<int, NewSearchTag> $newSearchTag
...
//    }
    ...
    {
        return HelperController::$webPageUrl . 'templates/' . $value;
    }
    ...
    {
        return "templates/{$this->getRawOriginal('cat_link')}";
    }

    /**
     * @return NewCategory|null Returns an associative array based on `NewCategory` model with tree data, or null if not found.
     */
    ...
    ): ?NewCategory
    {
        ...
        $callback = function () use ($select, $isStatus, $id, $parentId, $getChild) {
            ...
            $query = NewCategory::query()->when($select, fn($q) => $q->select($select));
            $query->whereId($id);
            ...
            $parentCat = null;
            if ($category->parent_category_id != 0) $parentCat = NewCategory::find($category->parent_category_id);
        ...
        ?int   $page = null): LengthAwarePaginator
    {
        ...
        $callback = function () use ($select, $filters, $limit, $page) {
            ...
            $query = NewCategory::query()->when($select, fn($q) => $q->select($select))->where('parent_category_id', 0);
            $query->where('total_templates', '>', 0);
     ...
     * @param NewCategory|null $parentCat
     * @param int[]|null $parentIds
     * @param array|null $select
     * @return Closure
     */

    private static function getChilds(?int $isStatus = null, ?NewCategory $parentCat = null, ?array $parentIds = null, ?array $select = null): Closure
    {
        $childQuery = NewCategory::query()->when($select, fn($q) => $q->select($select));
    ...
    private static function getChildsBackup(?int $isStatus = null, ?NewCategory $parentCat = null, ?array $parentIds = null, ?array $select = null): Closure
    {
        $childQuery = NewCategory::query()->when($select, fn($q) => $q->select($select));
