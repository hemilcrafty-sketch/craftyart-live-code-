...
class PaymentConfiguration extends Model
{
     ...
     * @return mixed
     */
    public static function getCredentialsByName(?Collection $paymentConfigs, string $scope, string $gateway): mixed
    {
        if (!$paymentConfigs) $paymentConfigs = self::getAllPaymentConfig();
        return $paymentConfigs->where('payment_scope', $scope)->Where('gateway', $gateway)->first();
    }
